<?php
class ABdev_revelance_flickr_stream extends WP_Widget {

	function ABdev_revelance_flickr_stream(){
		$widget_ops = array(
			'classname' => 'flickr-stream',
			'description' => __('Photostream from Flickr.', 'ABdev_revelance'),
		);
		$control_ops = array(
			'id_base' => 'flickr-stream',
		);
		parent::__construct('flickr-stream', __('Flickr stream', 'ABdev_revelance' ), $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance){
		...
		if($flickr_username && $number) {
			...
			if(is_object($stream) && isset($stream->user->id)) {
				...
				echo '<p>'.__('Invalid flickr username or connection error.', 'ABdev_revelance').'</p>';
			}
		}
		echo $after_widget;
	}
	...
	function form($instance){
		$defaults = array('title' => __('Photos from Flickr', 'ABdev_revelance'), 'flickr_username' => '', 'number' => 9);
		$instance = wp_parse_args((array) $instance, $defaults); ?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'ABdev_revelance');?></label>
			<input type="text" class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('flickr_username'); ?>"><?php _e('Flickr username:', 'ABdev_revelance');?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('No. of images:', 'ABdev_revelance');?></label>
...
function ABdev_revelance_flickr_stream_widget(){
	register_widget('ABdev_revelance_flickr_stream');
}

add_action('widgets_init', 'ABdev_revelance_flickr_stream_widget');