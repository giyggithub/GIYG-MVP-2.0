...
function tcvpb_service_box_tc_shortcode( $attributes, $content = null ) {
	...
	$return = '
		<div '.esc_attr($id_out).' class="tcvpb_service_box'.esc_attr($class).'">
			...
			$return .= ($link!='') ? '<a href="'.esc_url($link).'" target="'.esc_attr($target).'" class="tcvpb_icon_boxed"><i class="'.esc_attr($icon).'" '.esc_attr($icon_color).'></i></a>' : '<div class="tcvpb_icon_boxed"><i class="'.esc_attr($icon).'"></i></div>';
