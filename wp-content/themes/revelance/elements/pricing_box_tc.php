...
function tcvpb_pricing_box_tc_shortcode( $attributes, $content = null ) {
	...
	if($button_text != ''){
		...
		$button_out = '<div class="tcvpb_pricebox_feature tcvpb_pricebox_feature_button"><a href="'. $button_url .'" target="' . $button_target . '" class="'.$class_out.'">' . $button_text . $icon_out . '</a></div>';
	}

	static $count_priceboxes;
	$count_priceboxes++;
	return '
	<div '.esc_attr($id_out).' class="tcvpb_pricing-table-'.$style.' '.$featured_output.'">
